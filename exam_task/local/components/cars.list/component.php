...
/*!!*/
if (isset($arParams["TRAVEL_DATE"])) {
    $arParams["TRAVEL_DATE"] = ConvertDateTime($arParams["TRAVEL_DATE"], "YYYY-MM-DD", "ru");
}
if (isset($arParams["START_TRIP"])) {
    $arParams["START_TRIP"] = ConvertDateTime($arParams["START_TRIP"], "HH-MI-SS", "ru");
}
if (isset($arParams["END_TRIP"])) {
    $arParams["END_TRIP"] = ConvertDateTime($arParams["END_TRIP"], "HH-MI-SS", "ru");
}
/*!!*/
...
if (empty($arParams["IBLOCK_TYPE"])) {
    $arParams["IBLOCK_TYPE"] = "CONTENT_RU";
}
...
if ($this->startResultCache(false, array(($arParams["CACHE_GROUPS"] === "N" ? false : $USER->GetGroups()), $bUSER_HAVE_ACCESS, $arNavigation, $arrFilter, $pagerParameters))) {
    ...
    $rsUser = CUser::GetList(($by = "ID"), ($order = "DESC"),
        array("ID" => $USER->GetID(),),
        array("SELECT" => array("UF_CATEGORY",),
        )
    );
    if ($arUser = $rsUser->Fetch()) {
        foreach($arUser["UF_CATEGORY"] as $UF_ID){
            $rsEnum = CUserFieldEnum::GetList(array(), array("ID" => $UF_ID));
            $arEnum[$UF_ID] = $rsEnum->GetNext();
            $arUserCategory[] = $arEnum[$UF_ID]["VALUE"];
        }
    }
    /*!!*/
    ...
    $shortSelect = array('ID', 'IBLOCK_ID', 'PROPERTY_*');
    ...
    while ($row = $rsElement->Fetch()) {
        ...
        if ($ar_el = $elem->GetNextElement())
            if ($propComfort = $ar_el->GetProperty("COMFORT"))
                if (in_array($propComfort['VALUE'], $arUserCategory)) {
                    if (isset($arParams["TRAVEL_DATE"]) && $propBusy = $ar_el->GetProperty("BUSY")) {
                        if (SearchFreeTime($propBusy['VALUE'], $arParams["TRAVEL_DATE"], $arParams["START_TRIP"], $arParams["END_TRIP"])) {
...
/*!!*/
function SearchFreeTime($prop, $date, $start, $end)
{
    foreach ($prop as $time) {
        $ar_prop = explode(" ", $time);
        if ($ar_prop[0] == $date) {
            if (($ar_prop[1] >= $start && $ar_prop[2] <= $end)
                || ($ar_prop[1] < $start && $ar_prop[2] > $start)
                || ($ar_prop[1] < $end && $ar_prop[2] > $end)
                || ($ar_prop[1] < $start && $ar_prop[2] > $end))
                return false;
        }
    }
    return true;
